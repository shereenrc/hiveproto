...
with col1:
    st.image("images/logo.jpg", width=80)  # Adjust the image path and width
...
# Create a slideshow
selected_index = st.selectbox("TUTORIAL - Select a step to view:", range(len(image_paths)), format_func=lambda x: captions[x])
st.image(image_paths[selected_index], caption=captions[selected_index], use_column_width=True)
